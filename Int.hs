...
--------------
data SBind = CVar :-> CExp deriving (Show)
...
ura p x@(ces, r) y = map altRepr (ura' p x y)
  where
    ...
    altRepr xi@(cesi, ri@(RESTR rir)) = subClassCntr x xi

subClassCntr :: Class -> Class -> (Subst, Restr)
subClassCntr x@(ces, r) xi@(cesi, ri@(RESTR rir)) = (si, ri')
  where (True, si) = unify ces cesi
